            ...
            <fieldset>
              ...
              <div className="mini-list" style={{ padding: 0 }}>
                {settleable.map((row) => <label key={row.id} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12, color: "var(--paper-100)" }}>
                  ...
                  <span>RP {row.rpYear} <small style={{ color: "var(--sand-500)" }}>{row.period}</small> — {row.remaining > 0n ? <>reste <MoneyDisplay amount={row.remaining} />{row.penalties > 0n && <> (dont majorations <MoneyDisplay amount={row.penalties} />)</>}</> : "impayée (ancien registre)"}</span>
                </label>)}
              </div>
            </fieldset>
