...
export default async function EditNinjaPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const isOwner = !demoMode && ninja?.userId === session.userId;
  if (!demoMode && !canWrite && !isOwner) redirect("/access-denied");
  return <div className="page-wrap">
    ...
    <section className="panel" style={{ maxWidth: 640 }}>
      <SectionHeader title="Identité" description={canWrite ? "Prénom et nom restent obligatoires" : "Vous pouvez ajuster votre pseudonyme et votre clan — l’identité et le grade restent gérés par le service"} />
      <form action={updateNinja} className="form-grid">
        ...
        </div> : <><input type="hidden" name="firstName" value={ninja.firstName} /><input type="hidden" name="lastName" value={ninja.lastName} /></>}
        ...
        {canWrite && (ninja.status === "DECEASED" ? <div className="form-row">
          <label>État administratif<input value="Décédé" disabled /><input type="hidden" name="status" value="DECEASED" /></label>
          <label>Date du décès<input type="date" value={ninja.diedAt!.toISOString().slice(0, 10)} disabled /><input type="hidden" name="diedAt" value={ninja.diedAt!.toISOString().slice(0, 10)} /></label>
        </div> : <div className="form-row">
          <label>État administratif<select name="status" defaultValue={["ACTIVE", "INACTIVE"].includes(ninja.status) ? ninja.status : "ACTIVE"}><option value="ACTIVE">Actif</option><option value="INACTIVE">Inactif</option><option value="DECEASED">Décédé</option></select></label>
          <label>Date du décès<input type="date" name="diedAt" defaultValue={new Date().toISOString().slice(0, 10)} /></label>
        </div>)}
    ...
    {canWrite && <section className="panel" style={{ maxWidth: 640, marginTop: 12, borderColor: "rgba(169,79,63,.48)" }}>
